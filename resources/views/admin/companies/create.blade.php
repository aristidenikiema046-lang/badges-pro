    ...
    <div class="max-w-3xl mx-auto">
        ...
        @if(session('generated_slug'))
            <div class="mb-8 bg-emerald-50 border-2 border-emerald-500 p-6 rounded-xl shadow-lg border-dashed">
                ...
                <p class="text-sm text-emerald-700 mb-4 font-medium">Partagez ce lien avec les employés pour qu'ils créent leurs badges :</p>
                
                <div class="flex items-center gap-2 bg-white p-4 rounded-lg border border-emerald-200 shadow-inner">
                    <code id="company-link" class="text-emerald-600 font-mono font-bold flex-grow text-sm md:text-base italic">
                        {{ url('/register/' . session('generated_slug')) }}
                    </code>
                    <button onclick="copyToClipboard()" class="bg-emerald-600 text-white px-5 py-2 rounded-lg hover:bg-emerald-700 transition font-bold uppercase text-xs">
                        Copier
                    </button>
                </div>
            ...
            <form action="{{ route('companies.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="name" required placeholder="Ex: AgroNova SA" class="w-full border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-orange-500">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-1">Email Professionnel *</label>
                        <input type="email" name="email" required placeholder="contact@entreprise.com" class="w-full border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-orange-500">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-1">Responsable</label>
                        <input type="text" name="manager_name" placeholder="Nom du gérant" class="w-full border-gray-300 rounded-lg p-2.5">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-1">Téléphone</label>
                        <input type="text" name="phone" placeholder="+225..." class="w-full border-gray-300 rounded-lg p-2.5">
                    </div>
                </div>
