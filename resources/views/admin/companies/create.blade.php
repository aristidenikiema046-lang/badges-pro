...
<body class="bg-gray-100 p-4 md:p-8 font-sans">
                ...
                <form action="{{ route('companies.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    ...
                    <div class="border-2 border-orange-100 rounded-2xl p-6 bg-orange-50/30">
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            {{-- Zone d'aperçu --}}
                            <div class="flex flex-col items-center justify-start bg-white p-4 rounded-xl border border-orange-100 shadow-inner min-h-[400px] overflow-hidden">
    ...
    <script>
        ...
        function updateLivePreview() {
            ...
            // Appel AJAX vers la route Laravel
            fetch(`{{ url('/admin/preview-style') }}/${style}?color=${color}`)
                .then(response => {
                    if (!response.ok) throw new Error('Erreur 404 ou 500');
                ...
                .catch(err => {
                    renderContainer.innerHTML = "<p class='text-red-400 text-xs text-center'>Erreur : Vérifiez que la route preview-style est bien configurée.</p>";
                    loader.classList.add('hidden');
                    console.error(err);
                });
        }
        ...
        colorInput.addEventListener('input', updateLivePreview); // "input" est mieux que "change" pour le temps réel
        ...
        @if(session('generated_slug'))
            Swal.fire({
                title: 'Succès !',
                html: `
                    <p class="text-sm mb-4">Lien employé :</p>
                    <div class="bg-gray-100 p-3 rounded font-mono text-xs mb-4 break-all border border-orange-300">
                        {{ url('/register/' . session('generated_slug')) }}
                    </div>
                    <button onclick="navigator.clipboard.writeText('{{ url('/register/' . session('generated_slug')) }}')" class="bg-teal-600 text-white px-4 py-2 rounded font-bold text-sm">Copier le lien</button>
                `,
                icon: 'success',
