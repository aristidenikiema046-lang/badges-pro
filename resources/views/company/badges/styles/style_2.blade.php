    ...
    <div class="relative w-full h-full flex flex-col items-center">
        ...
        <div class="flex-grow flex flex-col items-center justify-center w-full px-6 text-center mt-2 z-10">
            <div class="space-y-0 w-full mb-3">
                <h2 class="text-xl font-bold uppercase tracking-tight text-gray-700 leading-none">{{ $employee->first_name }}</h2>
                <h1 class="text-3xl font-black uppercase tracking-tighter leading-none" style="color: {{ $mainColor }}">{{ $employee->last_name }}</h1>
            </div>
            
            <div class="w-full bg-gray-50 py-2 px-3 rounded-xl border border-gray-100 mb-4">
                <p class="text-[12px] font-black text-gray-800 uppercase tracking-wider truncate">{{ $employee->function }}</p>
                <p class="text-[10px] font-bold uppercase italic truncate" style="color: {{ $mainColor }}">{{ $employee->department ?? 'DÉPARTEMENT PRO' }}</p>
            </div>
